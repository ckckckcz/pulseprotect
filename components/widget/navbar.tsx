...
export default function Navbar() {
  ...
  return (
    <header key={authStateKey} className={`fixed top-0 left-0 right-0 z-50 bg-white transition-all duration-300 ${scrolled ? "bg-white shadow-md py-5" : "bg-transparent py-4"}`}>
      <div className="container mx-auto px-4 mb-4 flex justify-between items-center">
        ...
        <Link href="/" className="inline-flex items-center">
          ...
          <span className={`text-xl font-bold ${scrolled ? "text-teal-600" : "text-teal-600"}`}>SmartCity</span>
        </Link>

        {/* Navigation - Desktop */}
        <nav className="hidden md:flex items-center space-x-8">
          <Link href="/" className={`font-medium ${scrolled ? "text-gray-800 hover:text-teal-600" : "text-gray-800 hover:text-teal-600"} transition-colors`}>
            Beranda
          </Link>
          <Link href="/about" className={`font-medium ${scrolled ? "text-gray-800 hover:text-teal-600" : "text-gray-800 hover:text-teal-600"} transition-colors`}>
            Tentang Kami
          </Link>
          <Link href="/services" className={`font-medium ${scrolled ? "text-gray-800 hover:text-teal-600" : "text-gray-800 hover:text-teal-600"} transition-colors`}>
            Layanan
          </Link>
          <Link href="/contact" className={`font-medium ${scrolled ? "text-gray-800 hover:text-teal-600" : "text-gray-800 hover:text-teal-600"} transition-colors`}>
            Kontak
          </Link>
        </nav>
        ...
        <div className="md:hidden">
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`p-2 focus:outline-none ${scrolled ? "text-gray-800" : "text-gray-800"}`}>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-white border-t border-gray-200">
            <div className="container mx-auto px-4 py-4">
              <nav className="flex flex-col space-y-4">
                <Link href="/" className="font-medium text-gray-800 hover:text-teal-600 transition-colors p-2">
                  Beranda
                </Link>
                <Link href="/about" className="font-medium text-gray-800 hover:text-teal-600 transition-colors p-2">
                  Tentang Kami
                </Link>
                <Link href="/services" className="font-medium text-gray-800 hover:text-teal-600 transition-colors p-2">
                  Layanan
                </Link>
                <Link href="/contact" className="font-medium text-gray-800 hover:text-teal-600 transition-colors p-2">
                  Kontak
                </Link>
